...
public class Launcher extends TimerTask{
    ...
    public void run() {
        ...
        for (BaseScruper baseScruper : getScrupers()) {
            try {
                for (Event data : baseScruper.getData()) {
    ...
    private List<BaseScruper> getScrupers(){
        ...
        //storage.add(new SkScruperStandart()); // ссылки, но стремные
        //storage.add(new LeaderIdScruperStandart()); // ссылки
        ...
        //storage.add(new HackatonIoScruperStandart()); //ссылки
        //storage.add(new NetologyScruperStandart()); //не доделан
        //storage.add(new MskItStepScruperStandart()); // ссылки
